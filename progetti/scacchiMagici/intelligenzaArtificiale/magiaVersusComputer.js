...
/*_________________________PRIMO CLICK___________________________________*/
	...
	if(cont%2==0)
	{
		...
		if(colturno=="n")
		{
			pos=randomPosSel(document.getElementById(pos).innerHTML);
			setTimeout(function(){mossa(pos)},700);
		}
...
//_____________________________SECONDO CLICK___________________________//
	...
	else
	{
		...
		{
			pos=randomPos();
			setTimeout(function(){mossa(pos)},200);
		}	
	}
}
...
/*__________SELEZIONE POSIZIONE RANDOM PER IA__________________*/
function randomPos()
{
	var possibili = new Array();

	for(var i=0; i<8; i++)
	{
		for(var j=0; j<8; j++)
		{
			var lenstr=document.getElementById((i+1)+""+(j+1)).innerHTML.length;
			var colp=document.getElementById((i+1)+""+(j+1)).innerHTML.charAt(lenstr-7);

			if(document.getElementById((i+1)+""+(j+1)).innerHTML!='<img src="imm/vuota.png">' && colp=="n")
				possibili.push((i+1)+""+(j+1));
		}
	}
...
/*__________SELEZIONE MOSSA RANDOM PER IA__________________*/
function randomPosSel(questo)
{
	var possibili = new Array();
	...
	for(var i=0; i<8; i++)
	{
		for(var j=0; j<8; j++)
		{
			if(document.getElementById((i+1)+""+(j+1)).className=="selezionato" && document.getElementById((i+1)+""+(j+1)).innerHTML!=questo)
				possibili.push((i+1)+""+(j+1));
		}
	}
